import { contentToText, type ChatMessage } from './types'
...
export function buildHandoffSummary(args: {
  ...
  const lastCustomer = [...messages]
    .reverse()
    .find((m) => m.role === 'user' && contentToText(m.content).trim())
  ...
  const quote = truncate(contentToText(lastCustomer.content).trim(), MAX_QUOTE_LEN)
