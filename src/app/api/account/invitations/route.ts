...
//   on the list falls through to an empty base URL with a loud
//   console.warn — the resulting link is obviously broken
//   (`/join/<token>`, no scheme/host) rather than silently pointing
//   at someone else's domain. Operators who care about this attack
//   surface should set this to their canonical hostnames; everyone
//   else gets today's permissive behavior.
//
// There is no safe hardcoded fallback domain here — this app has
// no marketing/docs site of its own to fall back to, and guessing
// a domain would silently mis-route invite links. Set
// `NEXT_PUBLIC_SITE_URL` to avoid ever hitting this branch.
